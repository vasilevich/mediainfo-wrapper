...
function buildOutput(obj) {
  ...
  for (var i in obj.track) {
    if (obj.track[i]['$']['type'] === 'General') {
      var completeName = (obj.track[i]['Complete_name'] || obj.track[i]['CompleteName'])[0];
      out.file = completeName;
      out.general = {};
...
function buildJson(xml) {
  return new Promise(function (resolve, reject) {
    xml2js.parseString(xml, function (err, obj) {
      ...
      var mediaInfo = obj['Mediainfo'] || obj['MediaInfo'];
      if (!mediaInfo) return reject('Something went wrong');

      obj = mediaInfo;
      obj.File = obj.File || obj.media;
      var out = [];
